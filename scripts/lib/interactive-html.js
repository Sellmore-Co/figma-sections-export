// interactive-html.js — build explicit interactive helper partial shapes:
//
...
//   2. renderAccordionSection — shared landing.js data-accordion markup built
//      from Q&A content.
//
...
// Build shared landing.js accordion markup. `items` is [{ question, answer }];
...
function renderAccordionSection({ sectionId, items, headingText }) {
  ...
  const rows = items.map((item, i) => {
    ...
    return [
      '<div data-accordion-item data-open="false" class="py-[20px]">',
      '  <button type="button" data-accordion-toggle aria-expanded="false" class="flex w-full items-center justify-between gap-4 text-left font-semibold text-[18px] md:text-[20px] leading-[1.4] text-[var(--text-primary)]">',
      `    <span>{{ ${qVar} }}</span>`,
      '    <span data-accordion-icon aria-hidden="true" class="shrink-0 text-[28px] leading-none transition-transform duration-200">+</span>',
      '  </button>',
      `  <div data-accordion-panel class="mt-[12px] text-[16px] md:text-[18px] leading-[1.6] text-[var(--text-secondary)]">{{ ${aVar} }}</div>`,
      '</div>',
    ].join('\n');
  });
  ...
  const html = [
    `<!-- ${sectionId} — shared data-accordion markup (generated by export-section.js) -->`,
    ...
    heading,
    '    <div data-accordion data-allow-multiple="false" class="mt-[24px] border-t border-b border-[rgba(0,0,0,0.16)] divide-y divide-[rgba(0,0,0,0.16)]">',
    indent(rows.join('\n'), 6),
    '    </div>',
    '  </div>',
    '</section>',
