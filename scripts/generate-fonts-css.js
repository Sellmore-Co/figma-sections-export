...
function processCampaign(campaignDir) {
  ...
  const weightMap = new Map(); // family -> Set of numeric weights used with it
  for (const file of htmlFiles) {
    ...
    for (const [family, weights] of extractFamilyWeights(html)) {
      if (!weightMap.has(family)) weightMap.set(family, new Set());
      for (const w of weights) weightMap.get(family).add(w);
    }
  }
  ...
  if (existing !== null && !force && !isStub) {
    ...
    fs.writeFileSync(cssPath, buildFontsCss(familyMap, families, weightMap));
  ...
  for (const family of families) {
    ...
    required.push({ family, file: `${base}.woff2`, weights });
  }
...
function buildFontsCss(familyMap, families, weightMap = new Map()) {
...
`;
  ...
  const faces = families
    .map((family) => {
      const base = expectedFontFileBase(family);
      // The declared range must cover every weight the markup actually uses —
      // a weight outside the range silently falls back to ui-sans-serif (a
      // different family), which is far worse than the browser synthesizing
      // the weight from the shipped file.
      const used = [...(weightMap.get(family) || [])].sort((a, b) => a - b);
      const lo = Math.min(400, ...(used.length ? used : [400]));
      const hi = Math.max(700, ...(used.length ? used : [700]));
      const usedNote = used.length
        ? `weights used in markup: ${used.join(', ')} — split into per-weight @font-face blocks for the files you ship`
        : 'no weight utilities detected — split into per-weight @font-face blocks for the files you ship';
      return `@font-face {
  font-family: "${family}";
  src: url("../fonts/${base}.woff2") format("woff2"),
       url("../fonts/${base}.woff") format("woff");
  font-weight: ${lo} ${hi}; /* ${usedNote} */
  font-style: normal;
  font-display: swap;
}`;
    })
...
function buildRequiredTxt(required, missing) {
  ...
  for (const r of required) {
    ...
    const weightNote = r.weights && r.weights.length ? `   weights used: ${r.weights.join(', ')}` : '';
    lines.push(`  [${present}] ${r.file}   family "${r.family}"${weightNote}`);
  }
  lines.push('');
  return lines.join('\n');
}
